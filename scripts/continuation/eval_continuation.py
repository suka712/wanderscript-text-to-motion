...
def main():
    ...
    for name in args.ckpt_name:
        ...
        report[name] = (summarize(seams), summarize(goals), ns["cond_mode"])
        del tr
    ...
    with torch.no_grad():
        for i in idxs:
            ...
            o_seam.append(s)
            o_goal.append(g)
            # pure data check: B's own first pose vs A's ending pose, no VQ-VAE
            gt_seam.append(float(np.linalg.norm(
                d["gt_first_pose"].reshape(22, 3) - d["prefix_pose"].reshape(22, 3), axis=-1).mean()))
    report["ORACLE (GT tokens)"] = (summarize(o_seam), summarize(o_goal), None)
    report["DATA FLOOR (no VQ-VAE)"] = (summarize(gt_seam), None, None)
